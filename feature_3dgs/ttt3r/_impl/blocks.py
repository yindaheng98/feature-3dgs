...
class Attention(nn.Module):
    ...
    @staticmethod
    def _apply_rope(rope, tokens, positions):
        ...
        rotated = rope(tokens, safe_positions)
        valid = valid[:, None, :, None]
        return torch.where(valid, rotated, tokens)
    ...
    def forward(self, x, xpos, return_attn=False):
        ...
        if self.rope is not None:
            ...
            with torch.autocast(device_type="cuda", enabled=False):
                q = self._apply_rope(self.rope, q, xpos)
                k = self._apply_rope(self.rope, k, xpos)
            q = q.to(q_type)
            k = k.to(k_type)
    ...
    def forward(self, query, key, value, qpos, kpos, return_attn=False):
        ...
        if self.rope is not None:
            if qpos is not None:
                q = q.float()
                with torch.autocast(device_type="cuda", enabled=False):
                    q = Attention._apply_rope(self.rope, q, qpos)
                q = q.to(q_type) # [B, num_heads, Nq, C//num_heads] [1, 16, 768, 48]

            if kpos is not None:
                k = k.float()
                with torch.autocast(device_type="cuda", enabled=False):
                    k = Attention._apply_rope(self.rope, k, kpos)
