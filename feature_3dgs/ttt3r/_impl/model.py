...
from .camera import PoseEncoder
from .device import to_cpu, to_gpu
...
def load_model(model_path, device, verbose=True):
    ...
    # TTT3R/CUT3R checkpoints bundle OmegaConf/argparse objects in addition to
    # tensor weights, so PyTorch 2.6's default weights_only=True cannot load
    # them. These checkpoints are a trusted local dependency for this initializer.
...
class ARCroco3DStereo(CroCoNet):
    ...
    def __init__(self, config: ARCroco3DStereoConfig):
        ...
        croco_init = CrocoConfig.__init__ if CrocoConfig is not None else CroCoNet.__init__
        config.croco_kwargs = fill_default_args(config.croco_kwargs, croco_init)
        self.config = config
        self.patch_embed_cls = config.patch_embed_cls
        self.croco_args = config.croco_kwargs
        if CrocoConfig is not None:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kw):
        ...
        parent_from_pretrained = getattr(
            super(ARCroco3DStereo, cls), "from_pretrained", None
        )
        if parent_from_pretrained is not None:
            try:
                model = parent_from_pretrained(pretrained_model_name_or_path, **kw)
        ...
        raise FileNotFoundError(
            f"Local checkpoint not found and no Hugging Face loader is available: {pretrained_model_name_or_path}"
        )
