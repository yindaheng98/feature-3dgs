...
def prepare_gaussians(
    ...
    if not load_ply or not load_semantic:
        decoder.init_semantic(dataset, decoder=gaussians.load_decoder(load_decoder) if load_decoder is not None else None)
        gaussians.reset_encoded_semantics(dataset, mode=init_mode)
    return gaussians
